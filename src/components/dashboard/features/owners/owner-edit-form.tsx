...
import { updateOwnerAction } from '@/actions/dashboard/entities';
import {
  ownerEditSchema,
...
export function OwnerEditForm({ owner }: Props) {
  ...
  const form = useForm<OwnerEditFormData>({
    resolver: zodResolver(ownerEditSchema),
    defaultValues: {
      owner_name: owner.ownerName,
      owner_mobile_phone: owner.ownerMobilePhone,
      paypal_email: owner.paypalEmail ?? '',
      note: owner.note ?? '',
    },
    mode: 'onBlur',
  });
  ...
  const steps: WizardFormStep<OwnerEditFormData>[] = [
    {
      id: 'basic',
      title: t('steps.basic'),
  ...
  return (
    <div className="space-y-6">
      <HeaderInfo
        ...
        subtitle={owner.ownerMobilePhone}
      ...
      <WizardForm
        ...
        onComplete={async (values) => {
          const result = await updateOwnerAction(owner.id, values);
